...
import {
  ...
  ChevronRight, ChevronLeft, Star, CheckCircle2, Menu, X,
  Package, TrendingUp, Play, Quote
} from 'lucide-react'
...
const LOTTIE_URLS = {
  ...
  delivery: 'https://lottie.host/2c0e9a8d-d3c5-4a7e-8b2f-1e5c9d7a3b4f/delivery.json',
  analytics: 'https://lottie.host/55b6b9f1-a74f-4915-b948-5ebd1c95e4fb/animation.json',
  warehouse: 'https://lottie.host/43e50b7e-b978-4068-a0e7-3fc1a6f13c6d/warehouse.json',
  truck: 'https://lottie.host/f2a0a5d0-630c-4b9e-a3f8-2c00c62f1b9c/truck.json',
  globe: 'https://lottie.host/d45e9b39-93b7-4e39-bbd1-8d6bc8a5c6d9/globe.json',
    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2.5">
          ...
          <span className="text-lg font-bold text-white tracking-tight">Path Wounded</span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-3">
          <Link to="/login" className="text-sm font-medium text-slate-300 hover:text-white px-4 py-2 rounded-lg hover:bg-white/5 transition-all duration-200">
            Log in
          </Link>
          <Link to="/signup" className="text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 px-5 py-2 rounded-lg transition-all duration-200 shadow-lg shadow-blue-600/20 hover:shadow-blue-600/30">
            Get Started
          </Link>
        </div>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="md:hidden bg-dark-surface/95 backdrop-blur-2xl border-t border-white/5 overflow-hidden"
          >
            <div className="px-6 py-4 space-y-2">
              {links.map(l => (
                <a key={l.href} href={l.href} onClick={() => setMobileOpen(false)} className="block py-2 text-sm text-slate-400 hover:text-white">{l.label}</a>
              ))}
              <hr className="border-white/10 my-2" />
              <Link to="/login" onClick={() => setMobileOpen(false)} className="block py-2 text-sm text-slate-300 hover:text-white">Log in</Link>
              <Link to="/signup" onClick={() => setMobileOpen(false)} className="block py-2 text-sm font-medium text-blue-400">Get Started Free</Link>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-purple-600/10 rounded-full blur-[100px] animate-pulse" style={{ animationDelay: '1s' }} />
      ...
      <motion.div style={{ y: y1, opacity }} className="relative max-w-7xl mx-auto px-6 pt-32 pb-20 grid lg:grid-cols-2 gap-16 items-center">
        <div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="flex flex-col sm:flex-row gap-4">
            <Link to="/signup" className="group inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold px-8 py-3.5 rounded-xl transition-all duration-300 shadow-xl shadow-blue-600/25 hover:shadow-blue-600/40 hover:scale-[1.02]">
              Start Free Trial
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }} className="flex items-center gap-6 mt-10">
            <div className="flex -space-x-2">
              {['JD', 'MK', 'SR', 'AL'].map((initials, i) => (
                <div key={i} className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-blue-700 border-2 border-dark flex items-center justify-center text-[10px] font-bold text-white">{initials}</div>
              ))}
            </div>
            <div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.9, x: 40 }} animate={{ opacity: 1, scale: 1, x: 0 }} transition={{ delay: 0.4, duration: 0.8, ease: [0.22, 1, 0.36, 1] }} className="relative hidden lg:block">
          ...
          <div className="relative bg-dark-card/80 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-2xl">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs text-slate-500 ml-2">Path Wounded Dashboard</span>
            </div>
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[
                { label: 'Active Orders', value: '2,847', color: 'text-blue-400' },
                { label: 'Revenue', value: '$148K', color: 'text-emerald-400' },
                { label: 'Carriers', value: '500+', color: 'text-purple-400' },
              ].map(s => (
      ...
      <div className="max-w-7xl mx-auto px-6 relative">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-100px' }} variants={stagger} className="text-center mb-16">
          ...
          <motion.p variants={fadeUp} className="text-slate-400 max-w-2xl mx-auto text-lg">
            From booking to delivery, Path Wounded gives you complete visibility and control over your logistics operations.
          </motion.p>
        </motion.div>
    ...
    <section id="how-it-works" className="py-24 relative">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-8 relative">
          <div className="hidden md:block absolute top-24 left-1/6 right-1/6 h-px bg-gradient-to-r from-blue-600/50 via-blue-500/30 to-blue-600/50" />
          {steps.map((s, i) => (
            <motion.div
              key={s.num}
              initial="hidden"
            ...
            >
              <div className="relative inline-flex mb-6">
                ...
                <div className="absolute -top-2 -right-2 w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center text-xs font-bold text-white z-20 shadow-lg shadow-blue-600/30">
                  {s.num}
                </div>
              </div>
...
function Testimonials() {
  ...
  const testimonials = [
    { name: 'Sarah Mitchell', role: 'VP Operations, FreightCo', text: 'Path Wounded transformed our entire operation. We went from managing 50 shipments a week to 500, with the same team. The automation features alone saved us $200K in the first year.', rating: 5, initials: 'SM' },
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="max-w-3xl mx-auto">
          <div className="relative bg-dark-card/60 backdrop-blur border border-white/10 rounded-3xl p-8 md:p-12">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <p className="text-lg md:text-xl text-white leading-relaxed mb-8">"{testimonials[active].text}"</p>
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center text-sm font-bold text-white">
          ...
          <div className="flex items-center justify-center gap-3 mt-6">
            <button onClick={() => setActive(p => (p - 1 + testimonials.length) % testimonials.length)} className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-slate-400 hover:text-white hover:bg-white/10 transition-all">
              <ChevronLeft size={18} />
            </button>
            ...
            <button onClick={() => setActive(p => (p + 1) % testimonials.length)} className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-slate-400 hover:text-white hover:bg-white/10 transition-all">
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {plans.map((p, i) => (
            ...
            >
              {p.highlight && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-[11px] font-bold px-4 py-1 rounded-full shadow-lg shadow-blue-600/30">
                  MOST POPULAR
                </div>
              )}
    ...
    <section className="py-24 relative overflow-hidden">
      <div className="max-w-5xl mx-auto px-6 relative">
        <motion.div
          ...
          viewport={{ once: true }}
          variants={fadeUp}
          className="relative bg-gradient-to-br from-blue-600 to-blue-800 rounded-3xl p-12 md:p-16 text-center overflow-hidden"
        >
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <div className="relative z-10">
            ...
            <p className="text-blue-100/80 text-lg max-w-xl mx-auto mb-8">
              Join 500+ companies already using Path Wounded to streamline their freight operations. Start your free trial today.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link to="/signup" className="group inline-flex items-center gap-2 bg-white text-blue-700 font-semibold px-8 py-3.5 rounded-xl hover:bg-blue-50 transition-all duration-300 shadow-xl shadow-black/10 hover:scale-[1.02]">
                Start Free Trial
                <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
              </Link>
              <Link to="/login" className="inline-flex items-center gap-2 text-white/90 font-medium px-8 py-3.5 rounded-xl border border-white/20 hover:bg-white/10 transition-all duration-300">
                Schedule Demo
              </Link>
            </div>
          </div>
    ...
    <footer className="border-t border-white/5 pt-16 pb-8">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="border-t border-white/5 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-xs text-slate-500">&copy; 2026 Path Wounded. All rights reserved.</p>
