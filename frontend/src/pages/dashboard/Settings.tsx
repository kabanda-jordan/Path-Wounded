...
export default function Settings() {
  ...
  const profileMutation = useMutation({
    mutationFn: (d: { fullName?: string; companyName?: string }) => usersApi.updateProfile(d),
    onSuccess: (res) => { setUser(res.data.data); toast.success('Profile updated') },
    onError: (e: any) => toast.error(e.response?.data?.error?.message || 'Failed'),
  })

  const passwordMutation = useMutation({
    mutationFn: (d: { currentPassword: string; newPassword: string }) => usersApi.changePassword(d),
